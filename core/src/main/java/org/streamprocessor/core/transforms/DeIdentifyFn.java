...
public class DeIdentifyFn extends DoFn<Row, Row> {
  ...
  private static final Counter tokenCacheMissesCounter = Metrics.counter(
    "DeIdentifyFn",
    "tokenCacheMisses"
  );
  private static final Counter tokenCacheCallsCounter = Metrics.counter(
    "DeIdentifyFn",
    "tokenCacheCalls"
  );
  ...
  private static final com.github.benmanes.caffeine.cache.Cache<String, Object> tokenCache = Caffeine
    .newBuilder()
    .maximumSize(10000)
  ...
  public static Object getFieldToken(String tokenFullRef, Firestore db) {
    try {
      ...
      DocumentReference federatedTokenRef = db
        .collection("FederatedTokens")
        .document(federatedEntity + "#" + federatedIdentity);
      DocumentReference fieldTokenRef = federatedTokenRef
      ...
      // Document doesn't exist, create a new using transaction.
        ...
        // run a transaction
        ApiFuture<Object> futureTransaction = db.runTransaction(transaction -> {
          DocumentSnapshot fieldSnapshot = transaction.get(fieldTokenRef).get();
          if (fieldSnapshot.exists()) {
            return fieldSnapshot.get("token");
          } else {
            String federatedEntityToken = UUID.randomUUID().toString();
            DocumentSnapshot federatedSnapshot = transaction
              .get(federatedTokenRef)
              .get();
            if (!federatedSnapshot.exists()) {
              Map<String, Object> federatedEntityData = new HashMap<>();
              federatedEntityData.put("identity", federatedEntityToken);
              ...
              federatedEntityToken = federatedSnapshot.getString("identity");
            }
            ...
            switch (fieldType) {
              case "INT32":
                ...
                fieldData.put("value", Integer.parseInt(fieldValue));
                break;
              case "INT64":
                fieldToken = (long) (Math.random() * Long.MAX_VALUE);
  ...
  public void processElement(@Element Row row, OutputReceiver<Row> out)
    throws Exception {
    try {
      org.apache.beam.sdk.values.Row.FieldValueBuilder rowBuilder = Row.fromRow(
        row
      );
      ...
      for (int i = 0; i < beamSchema.getFieldCount(); ++i) {
        ...
        if (field.getOptions().hasOption("federated_identifier")) {
          String federatedEntity = field
            .getOptions()
            .getValue("federated_identifier", String.class);
      ...
      for (int i = 0; i < beamSchema.getFieldCount(); ++i) {
        ...
        if (
          (
            field.getOptions().hasOption("federated_identifier") &&
          ...
          if (federatedIdentity != null) {
            if (field.getType().getTypeName().equals(Schema.TypeName.STRING)) {
              ...
              String fieldToken = (String) tokenCache.get(
                federatedEntityRef +
                "#" +
                federatedIdentity +
                "#" +
              ...
              field.getType().getTypeName().equals(Schema.TypeName.INT32)
            ) {
              int fieldValue = row.getInt32(i);
              String fieldType = "INT32";
              int fieldToken = (int) tokenCache.get(
                federatedEntityRef +
                "#" +
                federatedIdentity +
                "#" +
              ...
              field.getType().getTypeName().equals(Schema.TypeName.INT64)
            ) {
              long fieldValue = row.getInt64(i);
              String fieldType = "INT64";
              long fieldToken = (long) tokenCache.get(
                federatedEntityRef +
                "#" +
                federatedIdentity +
                "#" +
              ...
              field.getType().getTypeName().equals(Schema.TypeName.DOUBLE)
            ) {
              double fieldValue = row.getDouble(i);
              String fieldType = "DOUBLE";
              double fieldToken = (double) tokenCache.get(
                federatedEntityRef +
                "#" +
                federatedIdentity +
                "#" +
