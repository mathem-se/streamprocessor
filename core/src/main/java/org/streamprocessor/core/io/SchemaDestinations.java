  ...
  public static class SchemaDestination extends DynamicDestinations<Row, Row> {
    ...
    public TableDestination getTable(Row row) {
      ...
      TimePartitioning timePartitioning = new TimePartitioning()
        .setField(
          schema.getOptions().getValueOrDefault("timePartitioningField", null)
        )
        .setExpirationMs(
          schema
            .getOptions()
            .getValueOrDefault("timePartitioningExpirationMs", null)
        )
        .setRequirePartitionFilter(
          schema
            .getOptions()
            .getValueOrDefault("timePartitioningRequirePartitionFilter", false)
        )
        .setType(
          schema.getOptions().getValueOrDefault("timePartitioningType", "DAY")
        );

      TableReference tableReference = new TableReference()
        .setProjectId(
          schema.getOptions().getValueOrDefault("projectId", tableProjectId)
        )
        .setDatasetId(
          schema
            .getOptions()
