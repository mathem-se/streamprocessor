...
class SchemaUtils {
  ...
  private static final Map<String, FieldType> FIELD_TYPES = ImmutableMap
  ...
  static Schema fromDataCatalog(
    com.google.cloud.datacatalog.v1beta1.Schema dcSchema
  ) {
  ...
  private static Field toBeamField(ColumnSchema column) {
    ...
    if (
      Strings.isNullOrEmpty(column.getMode()) ||
  ...
  static com.google.cloud.datacatalog.v1beta1.Schema toDataCatalog(
    Schema schema
  ) {
    com.google.cloud.datacatalog.v1beta1.Schema.Builder schemaBuilder = com.google.cloud.datacatalog.v1beta1.Schema.newBuilder();
  ...
  private static ColumnSchema fromBeamField(Schema.Field field) {
    ...
    if (fieldType.getTypeName().equals(Schema.TypeName.ARRAY)) {
      if (fieldType.getNullable()) {
        ...
        throw new UnsupportedOperationException(
          "Nullable array element type is not supported in DataCatalog schemas: " +
          fieldType
        );
      } else if (
        fieldType
          ...
          .getTypeName()
          .equals(Schema.TypeName.ARRAY)
      ) {
        throw new UnsupportedOperationException(
          "Array of arrays not supported in DataCatalog schemas: " + fieldType
        );
      }
      ColumnSchema column = fromBeamField(
        Field.of(field.getName(), fieldType.getCollectionElementType())
      );
      ...
      ColumnSchema.Builder colBuilder = ColumnSchema
        .newBuilder()
        .setType(getDataCatalogType(fieldType));
    ...
    switch (fieldType.getTypeName()) {
      ...
      case LOGICAL_TYPE:
        ...
        if (SqlTypes.TIME.getIdentifier().equals(logical.getIdentifier())) {
          ...
          throw new UnsupportedOperationException(
            "Unsupported logical type: " + logical
          );
        }
      case ROW:
        return "STRUCT";
