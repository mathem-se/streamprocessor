...
public final class CacheLoaderUtils implements Serializable {
  ...
  public static Schema getSchema(String linkedResource) {
    ...
    try (DataCatalogClient dataCatalogClient = DataCatalogClient.create()) {
      ...
      LookupEntryRequest request = LookupEntryRequest
        .newBuilder()
      ...
      String entity = linkedResource.substring(
        linkedResource.lastIndexOf("/") + 1
      );

      // Message level options
      Schema.Options schemaOptions = Schema.Options
        .builder()
      ...
      for (Tag tag : dataCatalogClient.listTags(entry.getName()).iterateAll()) {
        ...
        optionsMap.put(
          tag.getColumn().toLowerCase(),
          fieldSchemaOptionsBuilder.build()
        );
      }

      // Data catalog entry schema to list of beam Row schema fields
      List<Schema.Field> fieldList = SchemaUtils
        .fromDataCatalog(entry.getSchema())
        .getFields();
